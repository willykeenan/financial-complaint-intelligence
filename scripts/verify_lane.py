...
LANES = {
    ...
    "forward-holdout": "tests/test_calibration.py tests/test_metrics.py",
}
...
def main() -> int:
    if len(sys.argv) != 2 or sys.argv[1] not in LANES:
        print(
            "usage: verify_lane.py "
            "<data-pipeline|calibration-policy|evaluation-metrics|forward-holdout>"
        )
        return 2
    lane = sys.argv[1]
    root = Path(__file__).resolve().parents[1]
    result = subprocess.run(
        [sys.executable, "-m", "pytest", "-q", *LANES[lane].split()],
        cwd=root,
        check=False,
    )
